...
@dataclass
class APIConnector:
	settings_doc: ZKTecoBiometricSettings
	...
	def set_endpoint(self, endpoint) -> APIConnector:
	...
	def make_remote_call(self) -> dict:
		...
		try:
			...
			self.handle_request_failure(int_req_doc, self.parse_error(e))
			log_throw_error(f"HTTP Request to {self.absolute_url} failed to connect")
			...
			self.handle_request_failure(int_req_doc, self.parse_error(e))
			log_throw_error(f"HTTP Request failed: {e}")
	...
	def handle_request_failure(self, integration_req: IntegrationRequest, response: dict) -> None:
