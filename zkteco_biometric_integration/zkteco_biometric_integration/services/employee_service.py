...
def build_employee_filters(filters: dict | str | None = None) -> dict:
    ...
    query_filters = {
        field: filters[field] for field in EMPLOYEE_FILTER_FIELDS if filters.get(field)
    }
...
def resolve_employees(
    employees: list | str | None = None, filters: dict | str | None = None
) -> list[str]:
    """An explicit selection wins; otherwise everyone matching the filters."""
    employees = (
        frappe.parse_json(employees) if isinstance(employees, str) else employees
    )
    ...
    return frappe.get_all(
        "Employee", filters=build_employee_filters(filters), pluck="name"
    )
...
def search_employees(filters: dict | str | None = None, txt: str = "") -> list[dict]:
    ...
    or_filters = (
        [["name", "like", f"%{txt}%"], ["employee_name", "like", f"%{txt}%"]]
        if txt
        else None
    )
    ...
    return [
        {
            "value": row.name,
            "description": " / ".join(
                part for part in (row.employee_name, row.designation) if part
            ),
        }
        for row in rows
    ]
...
def sync_employees(settings_name: str, employees: list[str]) -> dict:
    ...
    settings: "ZKTecoBiometricSettings" = frappe.get_doc(
        "ZKTeco Biometric Settings", settings_name
    )
